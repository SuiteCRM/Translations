...
$mod_strings = array(
    ...
    'LBL_EMAIL_IMPORT_PER_RUN_THRESHOLD_DESC' => 'Threshold of emails to import per run. The default is 25. Each run processes the mailbox day by day within the configured timeframe, from the oldest to the most recent. All emails for each day are imported until the total number of emails imported in that run exceeds the threshold. Once the threshold is reached, the job stops and records the last day imported. The next run will resume from that day.',
    'LBL_EMAIL_IMPORT_TIMEFRAME_START' => 'Starting Point for New Mailbox Imports',
    'LBL_EMAIL_IMPORT_TIMEFRAME_START_DESC' => 'Period of time from which emails will start to be imported. For example, selecting 30 days will start with emails from 30 days ago and process forward from that point.',
    'LBL_EMAIL_IMPORT_FETCH_UNREAD_ONLY' => 'Only fetch unread emails during import',
);
